...
- **git = what shipped. ADRs = what was decided. cavemem = what was discussed.**
  A commit says *what* changed; `doc/adr/` says *why* it changed that way. Reach
  into cavemem only for a *why* that neither the commits nor the ADRs carry.
...
those are the traps waiting on the next task.
...
concrete artifact — a file, symbol, roadmap item, or "decision locked" — **verify
it against what git / roadmap / code show now.** Memories reflect what was true
when written; flag any drift explicitly rather than trusting them.
...
Commit messages and roadmap items point at docs — "ADR-0003", "VISION",
"roadmap". Read the *specific* referenced doc **only when the next move touches
it** (about to start the packing engine → read ADR-0003 and ADR-0004's decisions
and revisit triggers; changing product scope → read `doc/VISION.md`). Same
on-demand rule as cavemem: pulled by a question, not read by default.
