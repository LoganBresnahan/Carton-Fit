...
- [x] 39. Dogfood follow-ups, 16th run — one Claude Opus 5 pass (Cowork, over
      ...
      the reader's independent arithmetic** — five counts derived by hand
      before calling, per-plate weight to five figures, fill fraction checked
      in mm³, the tie reported as a tie — both paths identical to sixteen
      ...
      - [x] **`apply_preset` narrates a count whose weight the preset never
        ...
        cause the sentence never named. The reader asked for the binding note
        to say so; that note is one function on four surfaces (rule 5), and
        the panel already shows the override beside it. Derived: the surprise
        is announced where the client meets it (amendment 6's rule) —
        `apply_preset`'s reply gains `kept: { overriddenKinds, unitPart }`,
        the file-scoped state the preset left in force, the mirror of
      ...
      - [x] **Mesh volumes carry an unstated tessellation bias** (open; real;
        ...
        not, so the whole-file 13.229 lb is low by a fraction nobody bounds.
        Irrelevant at 38% of cap; not at 98%. The importer runs at
        occt-import-js's default deflection (nothing in `src/main/occt` sets
        one), so a bound would need that number, and a per-kind *curved*
        flag is derivable from the normals the importer already returns. A
        constant note is rule 2; a per-kind field is real work. **Decision
...
- [x] 37. Dogfood follow-ups, 14th run — one Claude Opus 5 pass (Cowork, over
      ...
      decimals (all five kinds reconciled), the tie at 35 lb reported as a tie,
      the hand-over at 100 lb — and both paths were byte-identical. Item 36's
      `withheldByCustomer: 1` was read *before* the reader worked it out, which
      ...
      - [x] **The bolt is flagged as mixed by float noise** (confirmed, wrong
        ...
        which `extentsMatch` in `kinds.ts` borrowed from the engine's
        containment tolerance. Float32 tessellation noise, four orders below
        any clearance, and it puts *bolt* in the same sentence as the nut,
        whose instances really are two 90° placements. Since the 7th run every
        reader has been told the bolt is mixed, and item 33's sub-item (the
        user's own "the warning makes the app look bad") was half noise.
        Derived: the alike test needs its own tolerance — one micron
        (`1e-3` mm), well above float32 noise on any part this app will see
        and well below any real orientation difference — not the engine's
      ...
      - [x] **The summary export leads with the density when none of the
        ...
        18.37 appears nowhere. The wire has the field — `countedWeightFrom:
        "override"`, the same rule in `estimate.ts` — and the summary does
        not read it; the CSV, which prints *Weight overrides* and no density
        line, is right. Fix: the summary derives the same
      ...
      - [x] **`set_inputs` says nothing about persisting** — refuted as
        ...
        amendment 12, ADR-0034 amendment 1's rejection). The step that was
        wrong: "indistinguishable from a considered default" — it is not,
        `unchangedAre: "earlier-session"` is exactly that distinction, and
        `setThisSession` is the disclosure. What is true and not a bug: every
